...
def evaluate(opt):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        with torch.no_grad():
            # for ind, (data, gt_depth, _, _, _, _) in enumerate(dataloader):
            for data in dataloader:
                # input_color = data.cuda() # [0, 1]
                input_color = data[("color", 0, 0)].cuda() # [0, 1]
                output = depth_decoder(encoder(input_color))

                inv_dist, _ = disp_to_depth(output[("inv_dist", 0)], opt.min_depth, opt.max_depth)
                if opt.use_computing_polar_phi:
                    ...
                    camera_fx = 0.58 * opt.width # fx
                    camera_fy = 1.92 * opt.height # fy
                ...
                pred_inv_dists.append(inv_dist.cpu()[:, 0].numpy())
                pred_phis.append(ori_phi.cpu()[:, 0].numpy())

        pred_inv_dists = np.concatenate(pred_inv_dists)
    ...
    for i in range(pred_inv_dists.shape[0]):
        gt_depth = gt_depths[i]
        gt_height, gt_width = gt_depth.shape[:2]

        pred_inv_dist = pred_inv_dists[i]
        pred_inv_dist = cv2.resize(pred_inv_dist, (gt_width, gt_height))
        pred_dist = 1 / pred_inv_dist
